...
export function useLocalDB() {
  ...
  useEffect(() => { setDb(readDB()); setLoaded(true); }, []);

  const persist = useCallback((newDb: LocalDB) => { setDb(newDb); writeDB(newDb); }, []);
  ...
  const addProduto = useCallback((produto: Produto) => {
    setDb((prev) => { const next = { ...prev, produtos: [...prev.produtos, produto] }; writeDB(next); return next; });
  }, []);
  ...
  return { db, loaded, saveValorHora, addProduto, updateProduto, deleteProduto, getProduto, listProdutos, persist };
}
